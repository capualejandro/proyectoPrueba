...
#		sleep(1);
		switch($_REQUEST["opc"]){
			...
			case "edimenu":
				...
				$caSql = "SELECT mod_codigo,mod_padre,mod_archivo,mod_descripcion,mod_opcion,mod_ordenpadre,mod_orden,mod_visible 
						  FROM modulo WHERE mod_codigo='$menu_codigo'";
				$arr = $db->GetArray($caSql);
				$numReg = count($arr);
				for($i=0; $i<count($arr); $i++){
					$arrEle[$i]['nom'] = $arr[$i][0];
...
/**/
				$caSql = "SELECT a.fun_nombre, a.fun_apellido, a.fun_telefono, a.fun_estado, b.usu_login 
						  ...
						  WHERE b.usu_codigo=$fun_codigo AND a.fun_identificacion=b.fun_identificacion";
/**/
/**
				$caSql="SELECT usuario.usu_codigo, usuario.usu_login, usuario.usu_fechacreacion, 
								funcionario.fun_identificacion, funcionario.fun_nombre, funcionario.fun_apellido, 
								funcionario.fun_telefono, funcionario.fun_estado 
								FROM usuario
								INNER JOIN funcionario ON usuario.fun_identificacion=funcionario.fun_identificacion
								AND funcionario.fun_codigo=$fun_codigo";
/**/
				$arr = $db->GetArray($caSql);
				$numReg = count($arr);
				for($i=0; $i<count($arr); $i++){
...
#				echo $html;
			...
			case "respag";
				...
				$caSql="SELECT proyectos.pro_codigo,contrato.cont_nomcontratista,MONTH(facturacion.fact_fech_pago),
								YEAR(facturacion.fact_fech_pago),SUM(facturacion.fact_valor_consign)
								FROM proyectos
...
#				echo var_dump($arrEle);
			...
			case "lista1";
				...
				$caSql="SELECT proyectos.pro_codigo,proyectos.pro_nombre,cliente.cli_nombre,proyectos.pro_fech_ini,
								proyectos.pro_fech_fin,proyectos.pro_porcentaje,proyectos.pro_valor_contrato
								FROM proyectos 
