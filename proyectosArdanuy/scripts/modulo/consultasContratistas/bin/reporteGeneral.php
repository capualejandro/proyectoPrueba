...
#-------------------------------------------------------------------------------------- General ------------------------------------------------------	
	$caSql="SELECT proyectos.pro_codigo,proyectos.pro_nombre,proyectos.pro_fech_ini,
					proyectos.pro_fech_fin,proyectos.pro_cliente,proyectos.pro_director,
					proyectos.pro_valor_contrato,proyectos.pro_valor_cont_sin_iva,proyectos.pro_porcentaje,
					proyectos.pro_porcentaje_antic,proyectos.pro_descripcion,proyectos.usu_login,contrato.cont_objeto,
					contrato.cont_nomcontratista,proyectos.pro_valor_contrato,proyectos.pro_valor_cont_sin_iva
					FROM proyectos
					INNER JOIN contrato ON contrato.pro_codigo=proyectos.pro_codigo
					WHERE proyectos.pro_codigo='$codProyecto'";
	...
	for($i=0; $i<count($arSocio); $i++){
		...
		$datSocio.='<tr><td>'.mayusculaInicial($nomSocio).'</td><td>'.mayusculaInicial($porSocio).' %</td></tr>';
	}
	$datSocio.='</table>';
	
	$caSql="SELECT cliente.cli_nombre,cliente.cli_nit,cliente.cli_direccion,
					pais.nom_pais,ciudad.nom_ciudad,cliente.cli_telefono
					FROM cliente
					INNER JOIN pais ON cliente.cli_pais=pais.cod_pais
					INNER JOIN ciudad ON cliente.cli_ciudad=ciudad.cod_ciudad
					AND cliente.cli_pais=ciudad.cod_pais
					AND cliente.pro_codigo='$codProyecto'";
	...
	$totalValorContrato=$valProSinIva+$valAdiSinIva;
	$totalValorContratoIva=$valProyecto+$valAdiones;
...
#	echo "$valEuro 884784800 Equivale a $valorAlejo";
	...
	$caSql="SELECT pro_codigo,pror_fech_ini,pror_fech_fin,pror_valor,pror_valor_sin_iva 
					FROM prorrogas WHERE pro_codigo='$codProyecto' ORDER BY pror_fech_ini";
...
#----------------------------------------------------------- Utilidades ----------------------------------------------------------------------------	
	$caSql="";
	$caSql="SELECT utilidades.pro_codigo, utilidades.soc_codigo, socio.soc_nombre,
					utilidades.uti_concepto,utilidades.uti_fecha,utilidades.uti_valor 
					FROM utilidades 
					INNER JOIN socio ON utilidades.soc_codigo=socio.soc_codigo
					AND socio.pro_codigo=utilidades.pro_codigo
					AND utilidades.pro_codigo='$codProyecto'
					ORDER BY 2";
#	echo $caSql;
...
#------------------------------------------------- Facturacion Servicios Profesionales -------------------------------------------------------------	
	$caSql="";
	$caSql="SELECT pro_codigo,fact_serv_prof_numero,fact_serv_prof_fech_pago,
					fact_serv_prof_abono,fact_serv_prof_valor,fact_serv_prof_concepto 
					FROM fact_serv_profesionales
					WHERE pro_codigo='$codProyecto'";
#	echo $caSql;
